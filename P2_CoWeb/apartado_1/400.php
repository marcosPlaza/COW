<html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <title>The Poké-Booking Service | Hey trainer, where do you wanna go?</title>

    <link rel="stylesheet" href="bootstrap-4.3.1_v2/css/bootstrap.min.css" type="text/css">
    <link href="fontawesome-free-5.15.2-web/css/all.css" rel="stylesheet">

    <script src="bootstrap-4.3.1_v2/js/jquery.min.js"></script>
    <script src="bootstrap-4.3.1_v2/js/popper.min.js"></script>
    <script src="bootstrap-4.3.1_v2/js/bootstrap.min.js"></script>

    <link rel="stylesheet" href="css/style_apartado1.css" type="text/css">
    <link rel="shortcut icon" href="images/pokeball.png">
</head>

<body>
    <div class="container mb-5">
        <div class="box">
            <div class="container-fluid" style="margin-left: 50px; margin-top: 50px; padding-top:50px">
                <h1>
                    Error <strong>400</strong>: Bad Request
                </h1>
                Some received value was wrong. You will be redirected to <a href="cliente.php">The PokeBooking Service</a>.
            </div>
            <div style="text-align:right">
                <img src="images/slowpoke.gif" alt="this slowpoke moves" width=400 />
            </div>
        </div>
    </div>
    <?php header("Refresh:5; url=cliente.php");?>
</body>

</html>